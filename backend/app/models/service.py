...
class Service(Base, UUIDPrimaryKeyMixin, TenantMixin, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, default=ServiceStatus.in_progress.value)
...
class ServiceItem(Base, UUIDPrimaryKeyMixin, TenantMixin, TimestampMixin):
    ...
    part_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("parts.id"), nullable=True, index=True)
